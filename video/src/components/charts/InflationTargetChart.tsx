...
const MARGIN: Margin = { top: 30, right: 30, bottom: 58, left: 78 };
...
export const InflationTargetChart: React.FC<ChartDimensions> = ({ width: W, height: H }) => {
    ...
    const abovePath = buildZonePath(MONTHS_X, ACTUAL, 2, 'above', xScale, yScale);
    const belowPath = buildZonePath(MONTHS_X, ACTUAL, 2, 'below', xScale, yScale);
    ...
    return (
        <svg width={W} height={H} style={{ display: 'block' }}>
            ...
            <g transform={`translate(${MARGIN.left},${MARGIN.top})`}>
                ...
                {/* 2% target line */}
                <line x1={0} y1={yScale(2)} x2={IW * targetProgress} y2={yScale(2)}
                    stroke={GOLD} strokeWidth={1.5} strokeDasharray="6 4" />
                <text x={8} y={yScale(2) - 7} fill={GOLD} fontSize={12} opacity={annotOpacity} fontWeight="600">2% target</text>
                {/* Transitory */}
                <g opacity={annotOpacity}>
                    <line x1={xScale(2021.4)} y1={yScale(5.4)} x2={xScale(2021.0)} y2={yScale(7.2)}
                        stroke={MUTED} strokeWidth={1} strokeDasharray="3 3" />
                    <text x={xScale(2021.0)} y={yScale(7.6)} fill={MUTED} fontSize={11} textAnchor="middle">"Transitory" assessment</text>
                </g>
                {/* Peak */}
                <g opacity={annotOpacity}>
                    <line x1={xScale(2022.5)} y1={yScale(9.1)} x2={xScale(2022.5)} y2={yScale(9.75)}
                        stroke={RED} strokeWidth={1} />
                    <text x={xScale(2022.5)} y={yScale(10.05)} fill={RED} fontSize={12} fontWeight="bold" textAnchor="middle">Peak: 9.1%</text>
                </g>
                <text x={IW / 2} y={IH + 46} textAnchor="middle" fill={MUTED} fontSize={13}>Year</text>
                <text transform="rotate(-90)" x={-IH / 2} y={-60} textAnchor="middle" fill={MUTED} fontSize={13}>CPI Inflation (%)</text>
            </g>
        </svg>
    );
};
...
function buildZonePath(
    xs: number[], ys: number[], target: number, side: 'above' | 'below',
    ...
    for (let i = 0; i < xs.length; i++) {
        const match = side === 'above' ? ys[i] > target : ys[i] <= target;
        if (match) {
            if (!inZone) { inZone = true; current = []; }
            current.push([xs[i], ys[i]]);
        } else if (inZone) {
            segments.push(current); current = []; inZone = false;
        }
    }
